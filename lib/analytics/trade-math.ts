...
import { type Trade, type TradingProfile } from "@prisma/client";
...
type TradeOutcomeValue = (typeof TRADE_OUTCOME)[keyof typeof TRADE_OUTCOME];
...
export function calculateRiskRewardRatio(
  input: Pick<TradeInput, "direction" | "entryPrice" | "stopLoss" | "takeProfit">
) {
  const risk =
    input.direction === "BUY" ? input.entryPrice - input.stopLoss : input.stopLoss - input.entryPrice;
  const reward =
    input.direction === "BUY" ? input.takeProfit - input.entryPrice : input.entryPrice - input.takeProfit;
...
export function determineTradeOutcome(pnlAmount: number) {
  if (pnlAmount > 0) {
    return TRADE_OUTCOME.WIN;
  }

  if (pnlAmount < 0) {
    return TRADE_OUTCOME.LOSS;
  }
...
export function detectTradeFlags({
  ...
  const overtradingFlag = Boolean(
    profile &&
      (dailyTradeCount + 1 > profile.dailyTradeLimit || weeklyTradeCount + 1 > profile.weeklyTradeLimit)
  );
