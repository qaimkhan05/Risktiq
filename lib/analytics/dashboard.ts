...
import { type DailyReflection, type Goal, type Trade, type TradingProfile } from "@prisma/client";
...
function groupStrategies(trades: Trade[]): StrategySummary[] {
  ...
  for (const trade of trades) {
    ...
    if (trade.outcome === TRADE_OUTCOME.LOSS) {
      current.losses += 1;
    }
...
function buildCoachInsights({
  ...
  if (overtradingTrades > 0) {
    insights.push(
      `${overtradingTrades} trade(s) breached your trade limits. Consider a hard stop after ${profile?.dailyTradeLimit ?? 0} trades.`
    );
  }

  if (revengeTrades > 0) {
    insights.push(`${revengeTrades} entries looked like revenge trading. Add a mandatory cooldown after losses.`);
  }

  if (averageRR < 1.4 && trades.length > 0) {
    insights.push(`Average risk-reward is ${averageRR.toFixed(2)}. Tighten stops or hold winners longer to improve expectancy.`);
  }

  if (bestStrategy) {
    insights.push(
      `${bestStrategy.strategy} is your strongest setup with ${bestStrategy.winRate}% win rate across ${bestStrategy.totalTrades} trades.`
    );
  }

  if (worstStrategy && worstStrategy.profit < 0) {
    insights.push(`Scale back ${worstStrategy.strategy}; it is your weakest strategy by net P&L this period.`);
  }

  if (emotionalTrades > 2) {
    insights.push("Elevated emotion markers were detected repeatedly. Journal before execution to reduce impulsive entries.");
  }

  if (monthlyGrowth < 0) {
    insights.push("Monthly growth is negative. Reduce frequency, focus on A+ setups, and review your losing clusters.");
  }

  if (averageDiscipline > 0 && averageDiscipline < 70) {
    insights.push("Reflection scores indicate discipline drift. Use the daily reflection and habit tracker more consistently.");
  }

  if (!insights.length) {
    insights.push("Execution quality looks stable. Maintain process consistency and keep reinforcing your best-performing strategy.");
  }
...
export function buildDashboardSnapshot({
  ...
  const currentMonthPnl = trades
    .filter((trade) => trade.tradeDate >= currentMonthStart)
  ...
  const monthlyGrowth =
    previousMonthPnl === 0 ? (currentMonthPnl > 0 ? 100 : 0) : ((currentMonthPnl - previousMonthPnl) / Math.abs(previousMonthPnl)) * 100;
  ...
  const reflectionDiscipline = safeAverage(reflections.map((reflection) => reflection.disciplineScore));
  const rulePenalty =
  ...
  const psychologyScore = Math.round(safeAverage(reflections.map((reflection) => reflection.psychologyScore)) || 88);
  const performanceScore = Math.round(safeAverage(reflections.map((reflection) => reflection.performanceScore)) || winRate);

  const activeGoals = goals.filter(
    (goal) => goal.status === GOAL_STATUS.ACTIVE || goal.status === GOAL_STATUS.COMPLETED
  );
  ...
  const weeklyReport = {
    ...
    weeklyPnl: Number(weeklyTrades.reduce((sum, trade) => sum + trade.pnlAmount, 0).toFixed(2)),
    strategyPerformance: groupStrategies(weeklyTrades),
    ruleViolations: weeklyTrades.reduce((sum, trade) => sum + trade.ruleBreakCount, 0),
    overtradingDays: [...new Set(weeklyTrades.filter((trade) => trade.overtradingFlag).map((trade) => format(trade.tradeDate, "yyyy-MM-dd")))].length,
    emotionalTradingCount: weeklyTrades.filter((trade) => trade.emotionalWarning).length
  };

  const monthlyReport = {
    totalTrades: monthlyTrades.length,
    monthlyPnl: Number(monthlyTrades.reduce((sum, trade) => sum + trade.pnlAmount, 0).toFixed(2)),
    topTrades: [...monthlyTrades].sort((a, b) => b.pnlAmount - a.pnlAmount).slice(0, 3),
    biggestMistakes: monthlyTrades.filter((trade) => trade.mistakeMade).slice(0, 4),
  ...
  if (!isToday(lastReflectionDate ?? new Date(0))) {
    reminderAlerts.push("Daily reflection has not been completed today.");
  }

  if (sortedTrades.length > 0 && !lastReflectionDate) {
    reminderAlerts.push("You have trades logged but no reflection history yet.");
  }

  const lastTradeDate = sortedTrades[sortedTrades.length - 1]?.tradeDate;
  if (lastTradeDate && !lastReflectionDate) {
    reminderAlerts.push("Recent trades are missing journal context. Add a reflection to keep your review loop complete.");
  }

  const aiInsights = buildCoachInsights({
    profile,
    trades,
  ...
  return {
    summary: {
      ...
      averageRiskRewardRatio: Number(safeAverage(trades.map((trade) => trade.riskRewardRatio)).toFixed(2)),
    ...
    habits: {
      averageDiscipline: Number((safeAverage(reflections.map((reflection) => reflection.disciplineScore)) || 0).toFixed(1)),
      averagePsychology: Number((safeAverage(reflections.map((reflection) => reflection.psychologyScore)) || 0).toFixed(1)),
      averagePerformance: Number((safeAverage(reflections.map((reflection) => reflection.performanceScore)) || 0).toFixed(1))
    }
  };
}
