...
import { getUserWorkspace } from "./dashboard-data";
...
function createPdfBuffer(title: string, sections: Array<{ heading: string; rows: ReportRow[] }>) {
...
function createWordBuffer(title: string, sections: Array<{ heading: string; rows: ReportRow[] }>) {
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>${escapeHtml(title)}</title><style>body{font-family:Arial,sans-serif;color:#0f172a;padding:24px;line-height:1.5}h1{font-size:26px;margin-bottom:24px}h2{font-size:18px;margin-top:24px;margin-bottom:10px;color:#0f766e}table{width:100%;border-collapse:collapse;margin-bottom:16px}th,td{border:1px solid #dbe4f0;padding:10px;text-align:left;vertical-align:top}th{background:#f3f8ff;width:28%}</style></head><body><h1>${escapeHtml(title)}</h1>${sections
    .map(
      (section) => `<section><h2>${escapeHtml(section.heading)}</h2><table><tbody>${ensureRows(section.rows)
        .map(
...
export async function buildWeeklyReportExport(userId: string) {
  ...
  const sections = [
    ...
    },
    {
      ...
      rows: ensureRows(
        week.strategyPerformance.map((s) => ({
          label: s.strategy,
          value: `${formatCurrency(s.profit)} | ${s.winRate}% win | ${s.totalTrades} trade(s)`,
        })),
        "Strategy",
  ...
  return {
    pdf: await createPdfBuffer("Risktiq Weekly Report", sections),
      ...
      },
      {
        ...
        rows:
          week.strategyPerformance.length > 0
            ? week.strategyPerformance.map((s) => ({ Strategy: s.strategy, Profit: s.profit, WinRate: s.winRate, TotalTrades: s.totalTrades }))
...
export async function buildMonthlyReportExport(userId: string) {
  ...
  const sections = [
    ...
    },
    {
      heading: "Top Trades",
      rows: ensureRows(
        month.topTrades.map((t) => ({
          label: `${t.assetName} (${t.strategyUsed})`,
          value: `${formatCurrency(t.pnlAmount)} | ${format(t.tradeDate, "dd MMM yyyy")}`,
        })),
        "Trades",
    ...
    },
    {
      ...
      rows: ensureRows(
        month.biggestMistakes.map((t) => ({
          label: `${t.assetName} (${t.strategyUsed})`,
          value: t.mistakeMade || "No note",
        })),
        "Mistakes",
  ...
  return {
    pdf: await createPdfBuffer("Risktiq Monthly Report", sections),
        ...
        rows:
          month.topTrades.length > 0
            ? month.topTrades.map((t) => ({
                ...
                PnL: t.pnlAmount,
                Date: format(t.tradeDate, "yyyy-MM-dd"),
                EmotionalBefore: t.emotionalBefore,
                EmotionalAfter: t.emotionalAfter,
              }))
