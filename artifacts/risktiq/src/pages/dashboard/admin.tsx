...
type AdminWorkspace = {
  ...
  customerJournals: Array<{
    id: string;
    email: string;
    name?: string | null;
    emailVerified: boolean;
    createdAt: string;
    profile: { fullName?: string | null; tradingStyle?: string | null; preferredStrategies?: string[] } | null;
    _count: { trades: number; reflections: number; goals: number };
    ruleBreaks: number;
    recentPnl: number;
    lastTradeAt: string | null;
    lastReflection: { reflectionDate: string; challenges: string; disciplineScore: number; psychologyScore: number; performanceScore: number } | null;
    trades: Array<{ id: string; assetName: string; strategyUsed: string; tradeDate: string; direction: string; outcome: string; pnlAmount: number; tradeNotes?: string | null }>;
  }>;
  recentPlatformTrades: Array<{
    id: string;
...
function AdminBody() {
  ...
  if (isLoading || !data) return <p className="text-sm text-muted-foreground">Loading admin data...</p>;
    ...
    <div className="grid gap-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card>
          ...
          <p className="mt-3 text-3xl font-semibold">{data.customerCount}</p>
        </Card>
        <Card>
          <p className="text-sm text-muted-foreground">Verified customers</p>
          <p className="mt-3 text-3xl font-semibold">{data.verifiedCustomerCount}</p>
        </Card>
        <Card>
          <p className="text-sm text-muted-foreground">Platform trades</p>
          <p className="mt-3 text-3xl font-semibold">{data.totalTradeCount}</p>
        </Card>
        <Card>
          <p className="text-sm text-muted-foreground">Platform reflections</p>
          <p className="mt-3 text-3xl font-semibold">{data.totalReflectionCount}</p>
        </Card>
      </div>
      ...
      <Card>
        ...
        <div className="mt-6 grid gap-5">
          {data.customerJournals.length ? (
            data.customerJournals.map((customer) => (
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-3 text-sm text-muted-foreground">
                      <span>Joined {formatDistanceToNow(new Date(customer.createdAt), { addSuffix: true })}</span>
      ...
      <Card>
        ...
        <div className="mt-6 overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {data.recentPlatformTrades.length ? (
                data.recentPlatformTrades.map((trade) => (
