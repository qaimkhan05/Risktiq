...
function TradesBody() {
  ...
  const { profile, trades, snapshot } = data;
  const strategies = profile?.preferredStrategies?.length
    ? profile.preferredStrategies
    : Array.from(new Set(trades.map((trade) => trade.strategyUsed).filter(Boolean)));
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
      <Card>
        ...
        <div className="mt-6">
          <TradeForm strategySuggestions={strategies} defaultTradeLimit={profile?.dailyTradeLimit ?? undefined} />
        </div>
      </Card>
