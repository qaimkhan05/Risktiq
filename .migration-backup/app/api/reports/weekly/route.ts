...
import { buildWeeklyReportExport } from "@/lib/reports";
...
export async function GET(request: Request) {
  try {
    ...
    if (format === "word") {
      return new NextResponse(Uint8Array.from(report.word), {
        headers: {
          ...
          "Content-Disposition": 'attachment; filename="weekly-report.doc"',
          "Cache-Control": "no-store"
        }
      });
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to export weekly report." },
      { status: 500 }
    );
  }
}
